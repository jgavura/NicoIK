...
def create_precision_plot(file_path, show_z_values=False):
    ...
    ax.set_xlim(left=-48, right=48)

    # 4. Plotting Data
    # Add Touchscreen Rectangle
    special_rect = Rectangle((-24, -53), 48, 27, 
                             linewidth=2, edgecolor='blue', 
                             facecolor='blue', alpha=0.1, 
                             linestyle='--', zorder=1)
    ax.add_patch(special_rect)
    ...
    ax.set_title('Exp - Vision - Unfocused Stereovision V1 - Grid', fontsize=20, pad=90)
    ax.tick_params(axis='both', labelsize=12)

    # Apply the custom handler to the legend
    if show_z_values:
        ax.legend(handles, labels, loc='upper right', bbox_to_anchor=(1.0, 1.23),
    ...
    # plt.show()
    plt.savefig('./experiment_vision/ex_vision_unfocused_stereo_v1_grid.png')

# Run the function
if __name__ == "__main__":
    create_precision_plot('./experiment_vision/center_stereovision_v1_grid_full.txt', show_z_values=True)